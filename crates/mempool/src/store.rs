...
mod tests {
    ...
    fn new_store(temp_dir: &TempDir) -> MempoolStore {
        let db_path = temp_dir.path().join("mdbx");
        MempoolStore::open(&db_path).expect("open store")
    }
    ...
    fn test_persistence_across_reopen() {
        ...
        {
            let store = MempoolStore::open(&db_path).unwrap();
            store.push(vec![7, 7]).unwrap();
            store.push(vec![8, 8]).unwrap();
        }

        let reopened = MempoolStore::open(&db_path).unwrap();
