...
A forwarding contract can make an ordinary internal `CALL` to the precompile address and remain valid, because the final hop is still direct. That path is covered in app-level verification tests in `crates/app/evm-execution/src/block_pipeline/tests/verification.rs`.
