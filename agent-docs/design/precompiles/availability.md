...
## Why
`whirlpool_precompiles(spec)` builds a `PrecompilesMap` from the built-in precompiles plus Whirlpool-owned entries in `crates/evm/precompiles/src/lib.rs:94-124`. `WhirlpoolEvmFactory` and `WhirlpoolEvmConfig::evm_with_env(...)` inject that map directly into `EthEvmBuilder` in `crates/evm/precompiles/src/lib.rs:142-163` and `crates/evm/app/src/config.rs:195-199`.

## Lifecycle model
- Genesis alloc and chain spec decide normal account/code/storage state; see `crates/evm/app/src/config.rs:29-94`.
- Precompiles are separate from genesis account deployment. They are runtime-registered execution hooks.
- Because the registry is attached when the EVM instance is created, the feature is present from the first executed block and in fresh `eth_call` contexts; see `crates/evm/app/src/config.rs:257-275`.
...
- Runtime registration keeps proposal and verification symmetric; see `crates/evm/app/src/executor.rs:761-800`.
