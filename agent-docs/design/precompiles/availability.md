...
- Because the registry is attached when the EVM instance is created, the feature is present from the first executed block and in fresh `eth_call` contexts; see `crates/app/execute/evm/app/src/config.rs:257-275`.
...
- Runtime registration keeps proposal and verification symmetric; see `crates/app/execute/evm/app/src/executor.rs:761-800`.
