...
## Summary
Whirlpool custom precompiles are node/runtime features, not contracts that must be deployed. The registry is constructed in `crates/precompiles/evm/src/lib.rs:94-124` and attached to the EVM builder in both `crates/precompiles/evm/src/lib.rs:142-163` and `crates/app/evm/app/src/config.rs:195-199`.
