...
## Summary
Whirlpool custom precompiles are node/runtime features, not contracts that must be deployed. The registry is constructed in `crates/precompiles/evm/src/lib.rs:94-124` and attached to the EVM builder in both `crates/precompiles/evm/src/lib.rs:142-163` and `crates/app/evm/execution/src/config.rs:195-199`.
...
- Make precompiles available everywhere the chain executes EVM logic, including proposal, verification, and RPC `eth_call`; see `crates/app/evm/execution/src/config.rs:195-199`, `crates/app/evm/execution/src/config.rs:257-275`, and `agent-docs/crates/rpc-eth.md`.
