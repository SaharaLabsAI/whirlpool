...
## Summary
Whirlpool custom precompiles are node/runtime features, not contracts that must be deployed. The registry is constructed in `crates/precompiles/evm/src/lib.rs:94-124` and attached to the EVM builder through `crates/precompiles/evm/src/factory_api.rs` and `crates/app/evm-execution/src/config/evm_adapter.rs`.
...
- Make precompiles available everywhere the chain executes EVM logic, including proposal, verification, and RPC `eth_call`; see `crates/app/evm-execution/src/config/evm_adapter.rs`, `crates/app/evm-execution/src/config/tests.rs`, and `agent-docs/crates/rpc-eth.md`.
