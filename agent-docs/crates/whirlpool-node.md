...
- Startup now validates any supplied `ChainSpec.genesis.alloc` against the shared native-token cap before the worker thread starts, so over-cap specs fail as `Err` instead of detached panics.
