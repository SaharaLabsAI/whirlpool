...
## Purpose
Canonical owner for strict DKG metadata/schema semantics used by Whirlpool block `extra_data`.

## Location
`crates/validators/dkg/`

## Owns
- DKG-aware canonical extra-data envelope: magic/version, section order, limits, strict decode rules, errors. Raw 32-byte legacy proposer-key carriers are rejected.
- `CanonicalExtraDataV1`, `FullDkgOutputV1`, `FullDkgV1`, `ReshareV1`.
- Codec/projection helpers: `encode_canonical_extra_data`, `decode_extra_data`, `project_raw_eth_extra_data`, `proposer_public_key_from_extra_data`.
...
## Boundary
Pure semantic crate. It must not depend on `app-traits`, `app-primitives`, `state`, `app-evm-execution`, or `evm-precompiles`. Runtime crates adapt local storage/config into DKG-owned input structs and traits.
...
- `app-primitives` wraps carrier-only helper calls for app block headers; `app-evm-state` and `rpc-eth` consume those helpers for block reconstruction/RPC projection.
