...
- `WhirlpoolEvmConfig` no longer carries ordered genesis validator-registry entries. Its named owners are:
  - `config/` owns static EVM adapter composition: `WhirlpoolEvmConfig`, `WhirlpoolRethEvmAdapter`, chain-spec access, `ConfigureEvm` forwarding, and Whirlpool precompile installation.
  - `context/proposer.rs` owns `ProposerRuntimeContext`, the node-local proposer public key; callers that exercise proposer paths must set `with_local_proposer_public_key(...)`. This is local context, not validator authority.
  - `context/dkg/` owns `DkgTransitionConfig`, a thin DKG transition-input composition. It must not own committed DKG history, active validator membership, or runtime validator reads.
...
- Reviewer entrypoints are now named by pipeline ownership zone:
  ...
  - `src/block_pipeline/` — `EvmApplication`, `Application` trait wiring, explicit `propose.rs` and `verify.rs` lanes, private `accounting/` helpers for fee and receipt derivation, and private `validators.rs` for loading non-empty state-backed validator entries plus DKG default-player schedules.
...
- `src/invariants/` is the crate-private invariant review surface. It is wired as `mod invariants;` rather than public API and names owner-specific invariant domains for reviewers, fuzzing work, and future formal verification.
  - `invariants::block_pipeline` names proposal/verification/accounting/epoch/DKG-call-site obligations while keeping orchestration in `block_pipeline`; DKG semantics stay delegated to `validators-dkg`, and epoch/community-pool/accounting write semantics stay delegated to `evm-precompiles`.
  - `invariants::post_handle` names receipt staging/finalization lifecycle obligations while `ReceiptStore` remains the staged/pending receipt state owner.
  - The invariant surface may expose only tiny pure behavior-preserving predicates; checks entangled with orchestration, I/O, or lower-crate semantics remain at owner call sites and are documented instead of moved.
...
- `app_evm_execution::context::proposer::ProposerRuntimeContext`
- `app_evm_execution::context::dkg::DkgTransitionConfig`
