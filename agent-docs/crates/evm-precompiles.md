...
- `src/invariants/{registry,call_boundary,epoch,fee_pool,community_pool,accounting,validators}.rs`: private pure semantic predicate tree for formal-verification-ready system-contract invariants; not re-exported as public API.
- `src/registry/{entry,registration,direct_call_guard,installed,build,runtime}.rs`: registered-precompile metadata, stateful registration trait, direct-call rejection, canonical installed list, checked map construction, and panic-on-invalid compatibility helpers.
...
- System-contract invariant checks are wired fail-closed at owner boundaries: registry uniqueness/builtin collision, direct-call/static-call boundaries, epoch advance and boundary write shapes, fee-pool withdraw value preservation, community-pool unlock distribution, burned-fee balance credit overflow, post-block accounting effect shape, and validators active-registry/proposer checks.
- The private invariant predicates are pure value checks and deliberately avoid `StateDb`, REVM internals, I/O, and app-local runtime types. Runtime modules map violations into existing public error carriers such as `RegistryError`, precompile `PrecompileError::other(...)`, `EpochBoundaryRuntimeError::EffectExtraction`, `PostBlockAccountingRuntimeError::{InvalidStoredValue,Execution}`, and `ValidatorsRuntimeError`.

## Verification
- Crate tests are source-adjacent `tests.rs` modules and cover registry construction, duplicate-address/builtin-collision rejection, dispatch routing, direct-call and static-call boundary enforcement, gas behavior, revert mapping, fee-pool withdraw/claim invariants, pure withdraw and epoch-advance transition planning, epoch semantic-core parity checks, private invariant predicate failures, validators runtime-state loading, burned-fee credit overflow rejection, and runtime-adapter `StateDb` load/apply plus fail-closed validate-before-commit coverage.
