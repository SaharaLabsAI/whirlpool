...
### Stage 5: State root computation
- **Owner**: `app-evm::EvmApplication`
- **Input**: `executor.into_state()` → `BundleState`
- **Action**: Compute state root from bundle state against parent state trie
- **Output**: Computed `state_root: [u8; 32]`
...
async fn verify(&self, parent: &EvmBlock, block: &EvmBlock) -> Result<ExecutionResult, EvmAppError> {
    ...
    let mut executor = self.evm_config.create_executor(state);
    let result = executor.execute_one(&evm_env, &ctx, &reth_block)?;

    let bundle_state = executor.into_state();
    let computed_root = compute_state_root(&bundle_state, &self.state_provider)?;
